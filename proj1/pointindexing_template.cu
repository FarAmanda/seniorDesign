...
//  A function that calculates the time between two timestamps
// Output returns the time, and prints the timestamp message.
...
// Void function for error handling
...
// Macrofunction for the above function.
#define HANDLE_ERROR( err ) (HandleError( err, __FILE__, __LINE__ ))


// a struct called point2d
// Assigns an x & y value to an object of the struct
struct point2d
{
...
// sets up the member function that transforms coordinate points into a single integer
// unsure why
struct xytor
{
...
int main(int argc, char *argv[])
{
	...
	/// num_points: stores number of variables, if arg[1] < 0 store 100000
	// run_lev = stores the lev variable from argv[2] , if argument <0 store 10
	/// prints out variables we are dealing with
	...
	// creates a vector 'da' the size of numpoints
	// creates timeval variables for record keeping
	...
	//  Establishes two pointers, dptr_points & dptr_cellids
	...
	// 1. Memory Space
	// 2. Proper Pointer Use
	// 3. any other errors relating to CUDA
	...
	// Generates random x,y coordinate points to be stored within each indicie of h_points
	...
	// input: d_points
	/// output: d_cellids
	/// use variable for lev and use output function
